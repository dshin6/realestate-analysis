...
- 분석 기간은 본문 상단에서 시작 월과 종료 월을 `YYYY-MM` 단위로 조절합니다.
- 층 구간: 1층, 저층(2~5층), 중층(6~15층), 고층(16층 이상)
- 전체 실거래 차트의 점 하나는 거래 한 건이며, 층 구간 차트는 진한 A/B/C 평균 막대와 투명한 개별 거래 점을 함께 표시합니다.
- 동·타입별 가격 비교 막대는 선택 기간의 산술평균을 사용합니다.
- 고층 대비 가격은 현재 선택 기간에서 같은 타입의 고층 평균을 100%로 두고 타입·층 구간 평균을 환산합니다.
- 시장 변동을 제외한 가격 차이는 타입(B타입 기준)과 층(고층 기준)을 별도 차트로 보여주며, 95% 신뢰구간과 거래 수는 막대 호버에서 확인할 수 있습니다.
- 실거래 가격지수는 분기·타입·동·실제 층을 함께 보정하며 거래 3건 이상인 최신 안정 분기를 100으로 둡니다. 이후 표본 부족 분기는 참고로만 표시합니다.
- 호가는 사용자가 직접 입력하며, 같은 타입의 실거래를 기준 분기·동·층 조건으로 환산해 적정가격과 범위를 계산합니다. 감정평가나 미래 가격 예측이 아닙니다.
- 타입별 거래 건수는 거래 활발도를 보여주지만 선호도를 직접 측정한 값은 아닙니다.
