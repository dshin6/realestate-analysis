...
- `realestate_analysis/config.py`: 대상 단지 식별정보, 면적 타입과 동별 최고층 설정
...
3. 취소 거래와 다른 단지를 제외하고 중복을 제거해 `data/cache/trades.json`에 저장한다.
4. 전용면적을 A/B/C 타입으로, 실제 층을 동별 상대 층 구간으로 분류한다.
5. 연도별 중앙값·프리미엄과 최근 3년 유사 거래 범위를 계산해 화면에 표시한다.
...
- Community Cloud 인스턴스가 재시작되면 실행 환경의 JSON 캐시가 사라져 전체 기간 API를 다시 수집할 수 있다.
- API 제공 필드와 행정구역 코드는 실제 응답 검증 뒤 조정될 수 있다.
- 동 미공개 거래는 동별 층 구간 분석에 제한이 있다.
