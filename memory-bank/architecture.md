...
- `realestate_analysis/analysis.py`: 타입·층 분류와 기간별 요약
- `realestate_analysis/valuation.py`: 헤도닉 가격지수, 유사 거래 현재가 환산, 호가 판정과 시간순 백테스트
...
9. 전체 유효 거래로 분기·타입·동·실제 층 보정 가격지수를 만들고, 거래 3건 이상인 최신 안정 분기를 100으로 사용한다.
10. 입력 호가를 같은 타입 유사 거래의 기준 분기·동·층 환산값과 비교해 적정가격·범위·판정·신뢰도를 표시한다.
...
- 가격지수: 로그가격의 분기·타입·동·실제 층 효과를 분리하고 최신 안정 분기를 100으로 정규화
- 호가 적정가격: 현재 조건으로 환산한 최대 30건 유사 거래의 시간 가중 중앙값, 범위는 가중 20·80백분위
...
- 희소 최신 분기 제외, 유사 거래 확장, 자료 부족 판정과 시간순 백테스트 단위 테스트
